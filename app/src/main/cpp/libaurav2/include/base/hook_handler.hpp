...
public:
    ...
    inline HookHandler& add_hook(const T orig, const U* hook_fn)
    {
        if constexpr (Config::USE_LOGGING) {
            spdlog::debug("adding hook for fn {}", typeid(T).name());
        }
    ...
    HookHandler& install_patch(const uintptr_t rel_addr, const std::vector<uint8_t>& bytes)
    {
        ...
        spdlog::get("global")->debug("replacing bytes [{:#04x}] with [{:#04x}] for addr {:#x}", fmt::join(orig_bytes, ","), fmt::join(bytes, ","), rel_addr);
#endif

        int res = patch_multiple_byte(patch_ptr, bytes);
        if (res != 0) {
            spdlog::warn("bytepatch for addr {} failed with errno {}!", rel_addr, res);
        }
